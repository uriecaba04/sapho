	...
	public function index(): string {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ((string)$this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/category')) {
				$owner[] = [
					'name'	   => $this->language->get('text_account_status_owner'),
					'href'     => $this->url->link('owner/account_status', 'user_token=' . $this->session->data['user_token']),
					'children' => []
				];
			}
            if($owner) {
				$data['menus'][] = [
					'id' => 'menu-dashboard',
					'icon' => 'fas fa-user',
					'name' => $this->language->get('text_owner'),
					'href' => '',
					'children' => $owner
				];
			}
